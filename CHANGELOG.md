...
- 新版（nx）项目类型等多选下拉会同时渲染 aria-live 镜像与可见标签，读取整个单元格得到两份相同文本，造成正确选中后仍报“回读不符”。改为读取唯一可见标签的 `title`，并补重复文本与多标签回归测试；真机预览与草稿暂存均通过。

### 新增

- `install-online.sh` 作为 curl 安装和更新共用入口：下载同一 Git ref 的技能包，复用现有安装器；已有私有运行环境先做无安装体检，必要时才运行 bootstrap。支持只更新指定 Agent 目录和固定 ref。

### 修复

**编辑既有草稿这条路在新版 UI 上首次真机走通，三个缺陷逐个暴露并修复**（此前只验证过新建路径）：

- **附件移除在编辑态找不到附件项 → 落成双附件**：新建表单上传后附件项外面包 `ul.file-list > li.file-list-item`，
  而**编辑既有记录时外面是 `div.file-list > div`，没有 ul/li**。按 `li.file-list-item` 写只在新建路径成立，
  编辑路径数出 0 个附件、跳过移除、再传一份。改用两种形态通用的 `.file-card-item`（上传完成证据同步改）。
- **已有正确值的下拉被重新点击 → 报「下拉无选项」**：编辑草稿时大多数单元格的值本来就对，脚本仍去重点一遍；
  新版这两个控件是 `ant-select-multiple`，已选项会被浮层过滤掉（实测浮层为空选不中），真点中了又等于
  **取消选中**、必填项静默变空。现在**值已对就不点**；值不对时照常选，但**选完回读**，不符即 fail-loud。
- **休假行没有「主要工作内容」输入框 → `fill` 超时 30s**：状态选「休假」后表单隐藏该列，
  而共享校验本来就允许休假行 content 为空。现在字段不存在且内容为空则跳过并记日志；
  内容非空却找不到输入框则 fail-loud——那是表单形态变了，不能当成"已经填过"。
...
- **同周已有非草稿记录时不再继续新建**（自查发现，真机复现）：`find_editable_draft` 原先对
  「本周没有记录」和「有记录但不可编辑」都返回 `None`，而调用方把 `None` 一律当成"去新建"。
  真机上记录已提交成「进行中」，脚本打印完「不可编辑」紧接着就新建并填满了整张表——只因当次
  没带 `--draft` 才没落库，带上就是同周第二条，正撞铁律「防重复」。现在非草稿一律抛错中止，
  要另建必须显式 `--new-record`。
- **列表网格没渲染出来时不再据此断定"本周没有草稿"**：`wait_for_list` 的返回值原先被两个调用点
  全部丢弃，超时后照样往下走 → 空列表 → 判定没草稿 → 新建，正是该函数注释里写的那个后果。
  现在只在"结论是没草稿"且网格确实没画出来时 fail-loud；网格画出来但零行（新用户）仍按
  正常路径新建，判据从「有没有行」改成「表头有没有画出来」。
- **不再把健康会话误报成「登录态过期」**（真机复现）：`assert_logged_in` 排在固定 3s 等待之后、
  等网格之前，新版首屏更慢时会报"页面未出现「报工周报」——多为登录态过期，重跑 --login"，
  而紧接着 `--keepalive` 返回会话有效。危害是把用户支去做完全错误的修复动作。现在先等网格再断言。
- **附件移除不再说谎**：旧版的 `.h3-icon-close` / `.anticon-close` 在新版是 **0 命中**（真机确认），
  原逻辑一次都没点就 break，却照样打印「已移除草稿中的旧附件」，然后上传新文件——草稿落成新旧
  附件并存。现在补上新版的 `svg.action-item.delete-action`，并改为以**附件项数量**为判据：
  移除后仍有剩余就 fail-loud，不落并存的草稿；本来就没有附件时如实说"跳过"。
- 上传完成证据补上新版形态 `li.file-list-item .title-item`，不再依赖"文件名出现在页面任意位置"
  这种弱判据（toast 或别的控件出现同名文本都会被当成成功）。
