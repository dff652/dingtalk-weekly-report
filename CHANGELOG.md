...
- **仿真 e2e 覆盖新版（nx）结构**：新增 `tests/mock_form_nx.html`（主 frame、`data-test-key`/
  `field`、虚拟滚动 `fixed-table`、只读 `ant-picker`、带搜索的关联 radio 列表、全屏新手引导、
  无空格「暂存」按钮），`run_mock_test.sh` 默认两套结构都跑，CI 无需改动即同时覆盖。
  仿真页刻意复刻了几个真机陷阱：引导遮罩要点两次才消失、日期面板固定停在非目标月（强制
  走表头翻页）、页面残留隐藏的旧下拉菜单、关联搜索命中词被 `highlight` 包裹导致 `innerText`
  不等于原文、附件点击处理器只挂在 svg 上。
- 填表时打印**探测到的 UI 形态**（`表单形态: legacy|nx`）：既是排障线索，也让仿真回归能按
  文件名断言「新版仿真页确实跑的是新版路径」——只断言字段值不够，新版页若探测失误回落到
  旧版分支照样会绿，那是「门还在响但已经不看新版了」的假绿。

### 修复

- mock 模式此前**硬编码走旧版分支**（`open_new_form` 直接返回主 frame、附件按 `mock` 而非
  UI 变体分流），导致新版结构的仿真页根本跑不到新版代码。现改为 mock 也做变体探测。

## [0.5.0] - 2026-09-07

### 修复

- **适配氚云新版（nx）表单，修复「30s 内表单未渲染」导致完全填不了表**：氚云把表单
  从 `/application/...` 迁到 `/nx/application/...` 并重写了前端，本工具的定位假设整条失效——
  表单不再在 `FormAdapter` iframe 里（改为主 frame 渲染）、控件身份从 `id` 换成
  `data-test-key`（顶层）/ `field`（子表列）、子表换成虚拟滚动的 `fixed-table`、
  日期控件从 `ant-calendar` 换成只读的 `ant-picker`、项目/产品名称从普通下拉换成
  带搜索框的关联 radio 列表、附件没有原生 `input[type=file]`、暂存按钮文字由
  `暂 存` 变成 `暂存`，另外新增了一层全屏新手引导遮罩会吃掉所有点击。
  **字段编码本身没变，所以用户 `config.json` 不需要改**；变的只是「用哪个属性去找它」。
  现在两套 UI 并存：运行时以「哪里能找到开始日期标签」探测变体（不靠 URL 猜，灰度期
  两套可能同时存在），旧版路径逐字保留，仿真 e2e 仍按旧版结构常态回归。
  真机验收：新建记录、8 行子表、附件上传与暂存全链通过；重跑能正确命中既有草稿改而不是
  再建一条。

  新版特有的几个坑已固化进代码注释与单测：
  - 新手引导 `.guide-wrapper` 不关掉时，报错是 "intercepts pointer events" 而不是
    「找不到元素」，极易误判成选择器写错；
  - 收起浮层**不能按 Escape**——真机实测会连整个新增弹窗一起关掉；
  - 附件上传的点击处理器挂在 `.upload-trigger-click` **里面那个 svg** 上，点外层 div
    既不报错也不弹文件选择器，因此按由内到外的候选逐个试；
  - 关联选择按 `.content` 的 `title` 属性精确匹配，不用 `inner_text`（命中词会被包进
    `<span class="highlight">`，且原文连续空格在 `white-space: pre` 下不可靠）；
  - 列表行状态在新版是**色块**不是文字，文字只在页脚图例里。现在运行时读图例建
    「颜色 → 状态」映射再翻译，不硬编码 RGB；认不出的颜色一律留空按「不是草稿」处理——
    编辑既有记录会覆盖真实申报，猜错一次就是改掉别人已生效的周报。
  - 列表首屏比旧版慢，固定 sleep 3s 时 `.tg-row` 还是 0，会把「有草稿」误判成「没有」
    进而多建一条撞唯一性判定；改为按元素轮询。
  - 实测「已生效」行的色块是 `background: none`（与页脚图例给的白色对不上），因此认不出的
    颜色在日志里明确写成「未能识别」而不是空字符串，避免出现「状态是「」」这种没法排查的提示。
