...
> **2026-09-24 对账**：当前 `VERSION` 与 CHANGELOG 最新版本条目均为 `0.5.0`，
> 但本地和 `origin` 均无 `v0.5.0` tag（最新 tag 为 `v0.4.1`）。因此 0.5.0 尚不满足下述
> 正式包版本门，不能将当前 `main` 或本轮未发布修复称为带 tag 的 0.5.0 发行物。
> 下次正式发版需先按实际发布基线修正版本/tag 对账；curl 的 `--ref` 只能指向已存在的 ref。
...
## 后续发布阻断（2026-07-28 已清）

原阻断两条，现状：

- ✅ 历史泄露处置完成（删库重建，旧对象实测全 404），且**远端安装只含脱敏版本已验证**——
  `run_release_acceptance.sh` 于 2026-07-28 首次 `RELEASE ACCEPTANCE PASS`，
  安装副本与提交逐字节一致、历史脱敏扫描 0 命中。
- ✅ 全部代码与文档以 Apache-2.0 发布，配套（版权主体 / SPDX 头 / 断言）已齐。

**发行链路本身已通**；剩下的不是阻断，而是可用性边界——见
[TESTING.md「装得上 ≠ 用得起来」](TESTING.md#装得上用得起来)。
