...
10. **真机结构性坑必须回灌 mock**。`mock_form.html` 已复刻行内同名滚动容器与子表分页
    （每页 10 行、新增后跳最后一页）；真机 bug 修复前先在 mock 复现负例（红）再修（绿），
    新测试场景用专用 fixture，不改共享 fixture 的语义（`test_core` 从共享 fixture 派生用例）。
...
| ⑩ | `fill_form.py` 子表填行 | 子表默认每页 10 行，第 11 行「新增」即翻页，可见行数骤降被误判为「新增失败」 | 高 | ✅ 已修（v0.4.1）：填行前 `fit_subgrid_page_size` 把每页条数调大到同屏 |
...
- `verify_attachment_uploaded` 在真实表单以页面出现附件名为完成证据——受控上传组件消费
  文件后会清空原生 input；这也与人工在 `20-filled-review.png` 上核对「附件已挂」
  **同一判据**。等不到就中止，不落缺附件的草稿。仿真表单没有异步上传完成信号，
  检查文件控件确实持有文件。原先的 `wait_for_timeout(4000)` 已删。
...
### ⑩ 子表分页导致 >10 行填表中止（2026-08-03 真机实战发现，v0.4.1 已修）

**症状**：真机 12 行周报（5 工作日 × 会议/开发 2 行 + 周末加班 2 行）填到第 11 行报
「点『新增』3 次后子表仍只有 3 行（期望 ≥11）」中止。此前所有周都 ≤10 行，从未触发。

**根因**：氚云子表是 ant-design 分页网格，每页默认 10 行。「新增」第 11 行时组件自动跳到
最后一页，`.ant-spin-container > .subgrid-sheet__row` 只渲染当前页——可见行数从 10 骤降到 3，
行计数与 `rows.nth(i)` 定位全部失真，被误判成「新增没生效」。不是按钮失效，也不是表单结构
变化：三次「新增」其实都成功了（分页「共13条」）。

**修复**（`fit_subgrid_page_size`）：填行前读分页「共N条」与当前每页条数，不够装下本周所有行
（编辑既有草稿时取两者较大值）就点开 `ant-pagination-options-size-changer` 选能装下的最小
选项；无分页控件（mock/旧表单）no-op；调整不生效或超过最大选项 fail-loud。mock 表单同步仿真
分页（新增后跳最后一页，复刻真机），负例复现与真机逐字一致的报错后转绿；`run_mock_test.sh`
改用 12 行 fixture 常态回归。

**连带教训**：把共享 5 行 fixture 直接改成 12 行会连坏 `test_core` 两个派生用例（周合计
52h 撞它的 40h 上限用例、双行/天让「删一行=缺一天」不再成立）——新场景开专用
`*_paged.json`，共享 fixture 的语义不动。
