...
### 踩坑 23：扫码成功仍等 300 秒——正向判据形成了首次配置死锁（Issue #3）

首次模板的 `form_texts.report_title` 为空，`configuration_plan` 却把它归入“登录后从真实表单
发现”；而 `looks_logged_in()` 在标题为空时必定返回 `False`。用户旅程要求先登录再
`--dump-record`，于是新用户即使完成扫码也无法被确认，必须等满 300 秒。这不是二维码或
`127.0.0.1` 的问题，是成功判据的前置数据在流程上永远拿不到。

另一个叠加点是 OAuth 扫码后未必落到 `form_url`。`--login-url` 已经会主动回到目标页，
`--login-web` / `--login` / `--login-sms` 原先却只检查扫码落地页；落到工作台时同样会误判失败。

修复遵守两条边界：

1. `report_title` 改为首次引导时由用户确认；缺失则在展示二维码前 fail-loud；
2. 二维码页保持不动，同一 Playwright context 新开探测页访问 `form_url`，只在目标页出现
   **可见**标题后保存 `state.json`。不把 URL 变化或 Cookie 变化当成功，不把 Cookie 交给
   `127.0.0.1` 状态页。
...
## 2026-08-03 真机实战：子表分页坑与 v0.4.1 回归

首次 12 行加班周（5 工作日 × 会议/开发 2 行 + 周末加班 2 行）真机落草稿，发现并修复
子表分页缺陷（完整根因与修法见 [REVIEW.md ⑩](REVIEW.md)）：

- **发现**：第 11 行报「点『新增』3 次后子表仍只有 3 行（期望 ≥11）」中止；99-error.png
  子表右下角显示「共13条 / 第2页 / 每页10条」——三次「新增」其实都成功了，是分页把可见
  行数变少。此前所有周 ≤10 行从未触发，mock 也没有分页仿真，属测试保真度盲区。
- **负例验证**：mock 补分页仿真（每页 10 行、新增后跳最后一页）后，临时禁用
  `fit_subgrid_page_size` 复现与真机**逐字一致**的报错；恢复修复后 12 行断言全过。
  「mock 复现成立」的判据就是报错逐字一致。
- **回归**：`run_mock_test.sh` 改用 12 行 `week_report_20260713_paged.json`（覆盖为 5 行
  fixture 的超集；5 行版仍由 test_core/smoke 使用）。test_core 52 项 / 填表边界 11 项 /
  版本 / 公开树 / `run_smoke.sh` 全绿。
- **真机复跑**：v0.4.1 经 zip → `install.sh --force` → md5 对账后，`--draft --confirmed`
  一次通过；日志「子表每页条数已调到 20（需 12 行同屏）」，「暂存成功」提示被正向观测，
  表单系统字段周总工时 52.00 与报告快照一致。
- 顺带补上 07-28 遗留：真机「点『暂存』后的正向确认」路径（`verify_draft_saved`）本次
  完整跑到。
...
| 真实氚云暂存验收（`verify_draft_saved` 那一段） | ✅ 已通过 | 2026-08-03 真机 12 行落草稿，「暂存成功」提示被脚本正向观测（见上节） |
