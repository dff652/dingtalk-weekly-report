...
- `config_version` 为 2；
- `vocabulary`、`form_fields`、`form_texts` 均来自本人或管理员确认，且
  `form_texts.save_draft` 明确是暂存而非提交；
...
- 恰有一条草稿：保留它；工具默认按「状态=草稿 且 开始日期=目标周周一」两道护栏定位并编辑；
- 没有同周记录：工具新建；
- 同周已有非草稿记录或多条记录：停止验收，由用户先确认真实状态；不得用 `--new-record`
  绕过唯一性判定。
...
- 钉钉草稿列表出现且只有一条目标周记录；
- 原有目标周草稿存在时，确认被更新的是该记录，没有新增重复记录；
