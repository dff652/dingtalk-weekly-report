...
- [ ] P3 氚云 OpenApi（**阻塞：缺 `EngineSecret`**；评估见下节）
...
**"工具没有提交能力"这条承诺的性质会变。** 今天它是**结构性**的：代码里根本不存在
那条路径。换成 API 之后会退化成**"我们选择不调那个接口"**——同一个凭据既能存草稿也能提交，
只隔着一个函数名。这是本项目对用户最核心的安全承诺，不能在重构里被顺手稀释。

配套要求（三条一起做，缺一条就不算数）：

1. 代码里**不出现**提交/审批端点的常量或路径拼装，不留"注释掉的"版本；
2. `tests/test_invariants.py` 必须继续通过，断言目标从"没有提交点击"改成
   "没有提交端点常量"——**改断言，不是删文件**；
3. `SECURITY.md` 里"脚本里不存在点提交的代码路径"这句要改写成 API 语境下**诚实**的表述，
   不能沿用旧措辞装作保证没变。

#### 两个待验未知（拿到 secret 后先验这两个，再决定要不要做）

| 未知 | 为什么是拦路的 |
|---|---|
| 附件上传是否有可用 API | 这类平台通常要**单独端点 + file token 两步**，未必对第三方开放。走不通则附件仍需浏览器 |
| 能否读到系统字段 | "周报唯一性判定"这类判据若只在表单层，API 侧就没法复现[编辑既有草稿]的两道护栏 |

#### 结论：并存，不是替换

即便全部验通，也建议**保留浏览器路径**：附件那段是最可能失败的一环，
留一条回落比赌 API 全能覆盖稳。配置里加一个 `backend: "api" | "browser"` 开关，
默认按能力探测，比一刀切换干净。

**下一步只有一件事**：向管理员要 `EngineSecret`。拿到之前本节不动。
