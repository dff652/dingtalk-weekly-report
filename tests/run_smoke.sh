...
test -f "$HOME/.claude/skills/dingtalk-weekly-report/LICENSE"
...
(
  cd /tmp
  python3 "$ROOT/skills/dingtalk-weekly-report/scripts/configure.py" --help \
    | grep -q "查看、校验"
...
DTWR_HOME="$ROOT/tests/fixtures" python3 \
  skills/dingtalk-weekly-report/scripts/configure.py --check \
  | grep -q "配置有效"
echo "OK configure check"
