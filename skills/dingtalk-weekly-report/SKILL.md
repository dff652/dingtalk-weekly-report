...
`install.sh`（装到 `~/.claude/skills/dingtalk-weekly-report`：默认 copy，维护仓 `--link`，升级 `--force`；
并清理旧名 `weekly-report`；有 `~/.codex` 时写桥接 prompt）、
...
**前提**：用户已用 zip/`install.sh`（或维护仓 `--link`）装好技能，故 `$SKILL` 可读。
本步只建运行态，不改技能包。

1. 问用户工作目录放哪（默认 `~/weekly-report-data`；只存个人数据 config/weeks/output/.venv，
   **不是代码目录**——代码在 `$SKILL`），`mkdir -p` 之。
2. 环境（PEP 668 时用 uv）：在 `$WORK` 下
...
5. 写 `~/.config/dtwr/root`（内容=$WORK 绝对路径；`mkdir -p -m 700 ~/.config/dtwr`）。

**技能升级**（用户拿到新 zip）：在解压目录再跑 `bash install.sh --force`，然后用新 `$SKILL` 继续；
`$WORK`/config/登录态保留不动。
