...
**用户文档（安装+每周怎么用）**：→ **[skills/dingtalk-weekly-report/USER_GUIDE.md](skills/dingtalk-weekly-report/USER_GUIDE.md)**（随 zip 分发；索引见 [docs/](docs/)）
...
| `skills/dingtalk-weekly-report/USER_GUIDE.md` | **安装与每周使用**（随 zip，给同事） |
...
| `docs/` | 文档索引（正文在技能包 `USER_GUIDE.md`） |
| `tests/mock_form.html` + `tests/run_mock_test.sh` | 仿真表单 e2e（改 fill_form 后必跑） |
| `tests/run_smoke.sh` | 打包+隔离 install+附件+仿真冒烟 |
| `output/` | 生成的附件与截图（gitignored） |

## 验证与测试（维护仓）

```bash
bash tests/run_smoke.sh       # pack + 隔离 install + gen_attachment + 拒绝覆盖 + mock e2e
bash tests/run_mock_test.sh   # 仅仿真填表 10 行断言
.venv/bin/python skills/dingtalk-weekly-report/scripts/fill_form.py --keepalive  # 真机会话（不落草稿）
```

| 覆盖 | 是否自动 |
|------|----------|
| zip 结构、多端 skill 安装、附件、粘贴块、仿真 e2e | ✅ `run_smoke.sh` |
| 真机 cookie 续期 | ✅ 可选 `--keepalive` |
| 真机 `--draft` / 钉钉提交 | ❌ 人工（防误造草稿） |
