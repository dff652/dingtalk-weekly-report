...
**自包含技能包与多用户安全**（2026-07-21 定稿）：`skills/weekly-report/` 按标准 Skill 结构
...
指针解析，缺省建议 `~/dingtalk-weekly-report`）彻底分离；脚本以 `DTWR_HOME`/cwd 定位工作目录。
...
| CLI | 安装 | 触发 |
|---|---|---|
| Claude Code | 把 `skills/weekly-report/` 整目录复制（或软链接）到 `~/.claude/skills/weekly-report/` | 新会话里 `/weekly-report`，description 也参与自动路由 |
| Codex CLI | Codex 无原生 SKILL.md 机制，用自定义 prompt 桥接：建 `~/.codex/prompts/weekly-report.md`，内容一句话——「严格阅读并执行 <技能包绝对路径>/SKILL.md」+ 三铁律提醒 | Codex 里 `/weekly-report`（仅手动斜杠触发，无自动路由） |
...
python3 skills/weekly-report/scripts/extract_week.py
# 2) 生成附件
python3 skills/weekly-report/scripts/gen_attachment.py weeks/week_report_YYYYMMDD.json
# 3) 登录态：每日 9:30 cron 自动 keepalive 滚动续命（crontab -l 可见；output/keepalive.log 看记录）。
#    仅当 keepalive 报「会话已失效」时才需人工续期：重新「打印内部二维码」→ 手机钉钉扫 →
.venv/bin/python skills/weekly-report/scripts/fill_form.py --login-url '<h3yun entry/auth 链接>'   # 链接本身 48h 有效
# 4) 半自动填表落草稿（填完自动截图 20-filled-review.png 可先核对）
.venv/bin/python skills/weekly-report/scripts/fill_form.py weeks/week_report_YYYYMMDD.json --draft
# 5) 钉钉里打开该草稿 → 人工核对 → 点「提交」（提交动作永远留给人）

# 回退路径（半自动不可用时）：print_form_rows.py 出粘贴块，手工填
python3 skills/weekly-report/scripts/print_form_rows.py weeks/week_report_YYYYMMDD.json
```
...
| 文件 | 作用 |
|---|---|
| `skills/weekly-report/SKILL.md` | 技能指令（Claude Code 入口，`$SKILL`/`$WORK` 双路径模型） |
| `skills/weekly-report/scripts/extract_week.py` | ① 工作日志 → `weeks/week_report_*.json` 草稿（拒绝覆盖已有文件） |
| `skills/weekly-report/scripts/gen_attachment.py` | ② json → `output/YYYYMMDD-YYYYMMDD本周工作总结与下周计划.xlsx` |
| `skills/weekly-report/scripts/print_form_rows.py` | ② json → 表单粘贴块（回退路径） |
| `skills/weekly-report/scripts/fill_form.py` | ③ Playwright 半自动填表（模式速查见下表） |
| `skills/weekly-report/scripts/xlsxlite.py` | 极简 xlsx 写入器（stdlib 手写 OOXML，零依赖） |
| `skills/weekly-report/scripts/dtwr_common.py` | 工作目录解析（`DTWR_HOME`/cwd，缺 config.json 即 fail-loud） |
| `skills/weekly-report/references/FIELDS.md` | **表单字段事实源**：氚云字段编码 + 合法枚举值 + DOM 坑 |
| `skills/weekly-report/assets/config.example.json` | 个人配置模板 |
...
- [x] **P2（路径 B）**：`fill_form.py` Playwright 半自动，**真机联调已通**（2026-07-21：token 免扫码登录、
      ...
      2. `.venv/bin/python skills/weekly-report/scripts/fill_form.py --login` → VSCode 打开 `output/shots/login.png` 手机钉钉扫码
         → 登录态存 `~/.config/dtwr/state.json`（0600，勿入 git）
      3. `.venv/bin/python skills/weekly-report/scripts/fill_form.py weeks/week_report_*.json` → 自动填表+传附件 → 截图核对
...
- [ ] **P3（路径 A，可选）**：氚云 OpenApi 直提（`POST https://www.h3yun.com/OpenApi/Invoke`）。
      字段编码已从导出文件拿到（见 `skills/weekly-report/references/FIELDS.md`），EngineCode 已知，
